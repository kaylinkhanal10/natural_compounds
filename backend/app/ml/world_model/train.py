...
from .metrics import compute_chemical_metrics, compute_protein_metrics, check_validity
...
def train_one_epoch(model, dataloader, criterion, optimizer, beta, device):
    ...
    # Track activations for simple diagnostic
    pred_counts = []
    ...
    for x_chem, x_prot in dataloader:
        ...
        with torch.no_grad():
            ...
            pred_counts.extend(preds.sum(dim=1).cpu().numpy())
    ...
    metrics = {
        ...
        'train_recon_prot': total_prot / len(dataloader),
        'train_kld': total_kld / len(dataloader),
        'train_pred_pos_mean': float(np.mean(pred_counts)) if pred_counts else 0.0,
        'train_pred_pos_median': float(np.median(pred_counts)) if pred_counts else 0.0
    }
    return metrics
...
def validate(model, dataloader, criterion, beta, device):
    ...
    with torch.no_grad():
        for x_chem, x_prot in dataloader:
            ...
            all_real_prot.append(x_prot)
            all_pred_prot.append(recon_prot)
            
    # Concatenate
    real_c = torch.cat(all_real_chem, dim=0)
    pred_c = torch.cat(all_pred_chem, dim=0)
    real_p = torch.cat(all_real_prot, dim=0)
    pred_p = torch.cat(all_pred_prot, dim=0)
    
    # Compute Advanced Metrics
    chem_metrics = compute_chemical_metrics(real_c, pred_c)
    prot_metrics = compute_protein_metrics(real_p, pred_p)
    validity = check_validity(pred_c, pred_p)
            
    metrics = {
        'val_total_loss': total_loss / len(dataloader),
        'test_recon_chem': total_chem / len(dataloader),
        'test_recon_prot': total_prot / len(dataloader),
...
def plot_metrics(history, save_dir):
    ...
    # 3. Protein Quality (PR-AUC)
    plt.figure(figsize=(10, 6))
    plt.plot(df['epoch'], df['test_prot_prauc'], color='purple', label='PR-AUC')
    plt.title('Protein Reconstruction Quality (PR-AUC)')
    plt.xlabel('Epoch')
    plt.ylabel('AUC')
    plt.legend()
    plt.savefig(os.path.join(save_dir, 'figures', 'prot_prauc.png'))
    plt.close()
...
def main():
    ...
    pos_weight_tensor = torch.tensor([20.0]).to(device)
    ...
    for epoch in range(epochs):
        ...
        # Merge metrics
        epoch_log = {'epoch': current_epoch, 'beta': beta, 'lambda_prot': lambda_prot}
        ...
        print(f"Epoch {current_epoch}/{epochs} | "
              ...
              f"PR-AUC: {val_metrics['test_prot_prauc']:.4f}")
        ...
        # Stopping Logic based on PR-AUC Stability (New robustness check)
        # Using PR-AUC > 0.3 as a decent threshold for collapse avoidance
        if val_metrics['test_prot_prauc'] > 0.8: # Very high bar, or use stability
            # Just standard save for now, rely on manual inspection of plots
            pass
