...
class MultiModalVAE(nn.Module):
    def __init__(self, chem_dim, prot_dim, latent_dim=32):
        super(MultiModalVAE, self).__init__()
        
        self.chem_encoder = ChemEncoder(chem_dim)
        self.prot_encoder = ProtEncoder(prot_dim)
        
        # Fusion: 128 (chem) + 128 (prot) = 256
    ...
    def forward(self, x_chem, x_prot):
        # Encode
        ...
        h_latent = self.fusion_net(h_fused)
        
        # Latent space
        mu = self.fc_mu(h_latent)
        logvar = self.fc_var(h_latent)
        z = self.reparameterize(mu, logvar)
        
        # Decode
        recon_chem = self.chem_decoder(z)
        recon_prot = self.prot_decoder(z)
        
        return recon_chem, recon_prot, mu, logvar, z
